...
# Visits per segment
df_register_segment_full = df_t2.groupby('segment')['REGISTERED_STORES'].sum().reset_index()
df_register_segment_full_sort = df_register_segment_full.sort_values(by='REGISTERED_STORES', ascending=False)
register_persegment = px.bar(df_register_segment_full_sort, x='segment', y='REGISTERED_STORES', color_discrete_sequence=['#ffcc00'])
